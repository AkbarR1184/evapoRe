...
millimeters (mm) \* `<resolution>` = 0.25° \*
...
In this introductory demo we will first download the GLDAS-CLSM data set.
...
**Note:**
  To download the latest version of `evapoRe`, please make sure you have the `devtools` package installed. You can install `evapoRe` by running the following command in R:
...
-   *data_name* is set to "all" by default, but you can specify the names of
...
Let's subset the GLDAS CLSM data set over Central Europe (2,28,42,58) for the
2001-2010 period, and inspect its content with `show_info`:
...
Let's crop our GLDAS CLSM subset to cover only Czechia with the respective
[shape
...
**NOTE:** Temperature data available at the moment is limited to monthly and covers only the land domain. The data source used for this information is TerraClimate. We will only perform PET calculation over 2001 to 2010.
...
Then, lets `brick` and subset temperature data over 2001 to 2010 to to this we will use `susbet_time` function which has three arguments *data*, *years*, and *autosave*.
...
tavg_subset <- raster::brick("terraclimate_tavg_land_19580101_20221231_025_monthly.nc") %>% subset_time(years = c(2001, 2010))
show_info(tavg_subset)
```
...
`pet` function is calculating PET from various *method*. "hs" for Hargreaves Samani method [@hargreaves1982estimating], "od" for Oudin method [@oudin2005potential], "mb" for McGuinness and Bordne method [@mcguinness1972comparison], "jh" for Jensen Haise method [@jensen1963estimating], and "br" for Baier and Robertson [@baier1965estimation].

#### Calculate PET by Oudin method

Let's calculate PET from Oudin method. Then, same as GLDAS CLSM we can subset it  over Central Europe for the 2001-2010 period, and Czechia and inspect its content with `show_info`:

```{r pet_calc, eval=FALSE}
pet_oudin_global <- pet(method = "od", tavg = tavg_subset)
...
[9] "max values : 219.7779, 175.7467, 185.8244, 190.6394, 223.7396, 225.7254, 237.2172, 220.6905, 186.1909, 181.9239, 191.0677, 201.9078, 208.3414, 172.4666, 185.1244, ... "
[10] "time       : 2001-01-01, 2010-12-01 (min, max)"
```
...
Let's generate the time series for our three different GLDAS CLSM data sets

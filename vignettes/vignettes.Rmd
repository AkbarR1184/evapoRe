...
`evapoRe` database hosts 10 different evapotranspiration data sets; one satellite-based, four reanalysis, and five hydrological model products. Their native specifications, as well as links to their providers, and their respective references are detailed in the following subsections. We have already homogenized, compacted to a single file, and stored them in a [Zenodo repository](https://doi.org/10.5281/zenodo.7990413) under the following naming convention:
...
* `<variable>` = Actual evapotranspiration (e)
...
Let's crop our fldas subset to cover only Czechia with the respective [shape file](https://geodata.ucdavis.edu/gadm/gadm4.1/shp/gadm41_CZE_shp.zip), and inspect its content with `show_info`:
...
First we need to download temperature data to to this we will use terraclimate tempereature data from the [Zenodo repository](https://doi.org/10.5281/zenodo.7990413):

```{r download_terraclimate, eval=FALSE}
download_terraclimate(variable = t, folder_path = ".")
```
This will returns terraclimate temperature (Tavg, Tmin, and Tmax) data in follwing name convention e.g., `terraclimate_tavg_land_19580101_20221231_025_monthly.nc`

`pet_calc` function is calculating PET from "hs" for Hargreaves Samani method [@hargreaves1982estimating], "od" for Oudin method [@oudin2005potential], "mb" for McGuinness and Bordne method [@mcguinness1972comparison], "jh" for Jensen Haise method [@jensen1963estimating], and "br" for Baier and Robertson [@baier1965estimation]. 
...
we will introduce significant enhancements to evapotranspiration (ET) database and potential evapotranspiration (PET) calculation methods. This expansion builds upon our existing temperature-based approach and incorporates a radiation-based PET calculation methods, along with an expanded range of temperature-based methods. Our aim is to provide users with a more comprehensive and accurate estimation of ET and PET, catering to a broader range of applications and requirements.

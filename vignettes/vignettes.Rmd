...
The `evapoRe` package developed as a complementary toolbox to the pRecipe package [@godoy2023precipitation], available at [https://cran.r-project.org/web/packages/pRecipe/index.html]. `evapoRe` facilitates the download, exploration, visualization, and analysis of evapotranspiration (ET) data. Additionally, evapoRe offers the functionality to calculate various Potential EvapoTranspiration (PET) methods.
...
Let's crop our GLDAS CLSM subset to cover only Czechia with the respective [shape
file](https://geodata.ucdavis.edu/gadm/gadm4.1/shp/gadm41_CZE_shp.zip), and inspect its content with `show_info`:
...
-   *folder_path* is set to "." by default. I.e., the current working
    directory.
-   *domain* is set to "raw" by default, but you can specify the domain
    of your interest only. E.g., "ocean" for ocean only data sets (For
    availability please check the [Data] section).

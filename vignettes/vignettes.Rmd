...
The `evapoRe` package developed as part of the ITHACA project at the
Faculty of Environmental Sciences, Czech University of Life Sciences
Prague, Czechia. Its primary purpose is to facilitate the download,
exploration, visualization, and analysis of evapotranspiration (ET)
data. Additionally, evapoRe offers the functionality to calculate
...
\* `<variable>` =  evapotranspiration (e) \* `<units>` =
...
**Note:**
  To download the latest version of `evapoRe`, please make sure you have the `devtools` package installed. You can install it by running the following command in R:
...
First we need to download temperature data. To to this we will use temperature data from the [Zenodo
...
This will download temperature data in following name convention e.g.,

`terraclimate_tavg_land_19580101_20221231_025_monthly.nc`

`pet_calc` function is calculating PET from various *method*. "hs" for
Hargreaves Samani method [@hargreaves1982estimating], "od" for Oudin
...
oudin method (Global, Central Europe, and Czechia), and inspect its
first 12 rows:

```{r oudin_global_ts, eval=FALSE}
pet_oudin_global_ts <- make_ts(pet_oudin, name = "terraclimate")
...
![](plots/map_fldas_global.png){width="8.4cm" height="5cm"}
...
![](plots/map_fldas_ceu.png){width="8.4cm" height="5cm"}
![](plots/map_oudin_ceu.png){width="8.4cm" height="5cm"}
...
![](plots/map_fldas_cze.png){width="8.4cm"
height="5cm"}![](plots/map_oudin_cze.png){width="8.4cm" height="5cm"}
...
ggarrange(p01, p02, ncol = 1)
# Plotting subsets
p01<- plot_line(fldas_subset_ts, var = "Evapotranspiration")
p02<-plot_line(pet_oudin_subset_ts, var = "Potential Evapotranspiration")
ggarrange(p01, p02, ncol = 2)
# Plotting cz

p01<- plot_line(fldas_cz_ts, var = "Evapotranspiration")
p02<- plot_line(pet_oudin_cz_ts, var = "Potential Evapotranspiration")
ggarrange(p01, p02, ncol = 2)
...
p01<- plot_heatmap(fldas_global_ts)
p02<- plot_heatmap(pet_oudin_global_ts)
ggarrange(p01, p02, ncol = 1)
...
# Plotting globals
p01<- plot_box(fldas_global_ts, var = "Evapotranspiration")
p02<- plot_box(pet_oudin_global_ts, var = "Potential\n Evapotranspiration")
ggarrange(p01, p02, ncol = 2)

# Plotting subsets

p01<- plot_box(fldas_subset_ts, var = "Evapotranspiration")
p02<- plot_box(pet_oudin_subset_ts, var = "Potential\n Evapotranspiration")
ggarrange(p01, p02, ncol = 2)

# Plotting cz

p01<- plot_box(fldas_cz_ts, var = "Evapotranspiration" )
p02<- plot_box(pet_oudin_cz_ts, var = "Potential\n Evapotranspiration" )
ggarrange(p01, p02, ncol = 2)
...
p01<- plot_density(fldas_global_ts, var = "Evapotranspiration")
p02<- plot_density(pet_oudin_global_ts, var = "Potential Evapotranspiration")
ggarrange(p01, p02, ncol = 2)


# Plotting subsets

p01<- plot_density(fldas_subset_ts, var = "Evapotranspiration")
p02<- plot_density(pet_oudin_subset_ts, var = "Potential Evapotranspiration")
ggarrange(p01, p02, ncol = 2)


# Plotting cz

p01<- plot_density(fldas_cz_ts, var = "Evapotranspiration")
p02<- plot_density(pet_oudin_cz_ts, var = "Potential Evapotranspiration")
ggarrange(p01, p02, ncol = 2)
...
![](plots/summary_fldas.png){width="95%"}
...
![](plots/summary_oudin.png){width="95%"}

...
#'
#' `my_palette` is a character vector of two colors specifying the color for  
#' extreme events and broader events, respectively.
#'
#' `event_color` defines the color for marking event periods along the x-axis.
#'
#' `shapes` is a numeric vector of three shape codes for:  
...
#' @param my_palette Character vector of 2 colors: (1) extreme, (2) event
#' @param event_color Character; color for x-axis event markers (default = "#a9cce0")
...
#' plot_exeves(dt)
#' }
plot_exeves <- function(
    x,
    my_palette = c("#4D648D", "#337BAE"),
    event_color = "#a9cce0",
    title = NULL,
    shapes = c(0, 16, 15)
) {
  stopifnot("data.table" %in% class(x))
  if (length(my_palette) != 2) stop("my_palette must have 2 colors.")

...
#'
#' The function \code{mcguinness_bordne} computes PET using the McGuinness and Bordne method.
#'
#' @details
...
setMethod("mcguinness_bordne", "Raster",
          function(x){
            ...
            dummie_pet <- foreach(index = seq_along(t_idx)) %dopar% {
              ...
              dummie_mb <- dummie_re * (dummie_ta + 5) / (68 * lambda)
              dummie_mb <- calc(dummie_mb, function(x) { x[x < 0] <- NA; x })
              return(dummie_mb)
            }

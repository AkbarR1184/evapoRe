...
#' @examples
#' \dontrun{
#' #' subset_spacetime("jra55_e_mm_land_195801_202112_025_monthly.nc",
...
subset_spacetime <- function(data, years, bbox, autosave = FALSE){
  ...
  if (checker$length == 8) {
    ...
    warning("This is not evapoRe data")
    nc_out <- sub(".nc.*", "", nc_in)
  ...
  if (is.character(dummie_dates) | is.numeric(dummie_dates)) {
    ...
    if (length(dummie_dates[dummie_dates == "00"]) >= 1) {
      ...
      if (grepl("gleam", nc_out)) {
        dummie_origin <- "1980-01-31 00:00:00"
      } else if (grepl("era5", nc_out) | grepl("terraclimate", nc_out)) {
        dummie_origin <- "1900-01-01 00:00:00"
      } else {
        dummie_origin <- "1800-01-01 00:00:00"
      }
